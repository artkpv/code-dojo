// Exercise 6-2. Write a program that reads a C program and prints in
// alphabetical order each group of variable names that are identical in the
// first 6 characters, but different somewhere thereafter. Don't count words
// within strings and comments. Make 6 a parameter that can be set from the
// command line. 
//
// NExt: make it parse only variables in c. Not all words.

#include <stdio.h>
#include <ctype.h>
#include <string.h>
#include <assert.h>
#include "c_parser.c"

struct tnode { /* the tree node: */
	char *word; /* points to the text */
	int count; /* number of occurrences */
	struct tnode *left; /* left child */
	struct tnode *right; /* right child */
};

#define MAXWORD 100
#define DEFAULT_CHARS_NUMBER 6

struct tnode *addtree(struct tnode *, char *);
void treeprint(struct tnode *);
int getword(char *, int);

/* word frequency count */
int main(int argc, char *argv[])
{
	int charsNumber = DEFAULT_CHARS_NUMBER;
	while(argc-- > 1) {
		argv++;
		if((*argv)[0] == '-' && isdigit((*argv)[1]))
			charsNumber = atoi((*argv + 1));
		else if (strstr(*argv, "--help") || strstr(*argv, "-h") || strstr(*argv, "/?")) {
			printf("Usage: varscount [-charsNumber] \n  where charsNumber - number of chars to group variables\n");
			return 0;
		}
	}
	assert(charsNumber > 0);

	struct tnode *root;
	char word[MAXWORD];
	root = NULL;
	while (getword(word, charsNumber) != EOF) {
		if(isalpha(word[0])) {
			root = addtree(root, word);
		}
	}
	treeprint(root);
	return 0;
}

struct tnode *talloc(void);

/* addtree: add a node with w, at or below p */
struct tnode *addtree(struct tnode *p, char *w)
{
	int cond;
	if (p == NULL) { /* a new word has arrived */
		p = talloc(); /* make a new node */
		p->word = strdup(w);
		p->count = 1;
		p->left = p->right = NULL;
	} else if ((cond = strcmp(w, p->word)) == 0)
		p->count++; /* repeated word */
	else if (cond < 0) /* less than into left subtree */
		p->left = addtree(p->left, w);
	else /* greater than into right subtree */
		p->right = addtree(p->right, w);
	return p;
}

/* treeprint: in-order print of tree p */
void treeprint(struct tnode *p)
{
	if (p != NULL) {
		treeprint(p->left);
		printf("%4d %s\n", p->count, p->word);
		treeprint(p->right);
	}
}

#include <stdlib.h>

/* talloc: make a tnode */
struct tnode *talloc(void)
{
	return (struct tnode *) malloc(sizeof(struct tnode));
}

int getword(char *word, int lim)
{
	int c, getch(void);
	void ungetch(int);
	char *w = word;

	while(isspace(c = getch())) 
		;

	if(c == '"')
		skip_string_constants();

	if(c == '/')
		skip_comments();

	if (c != EOF)
		*w++ = c;
	if (!isalpha(c)) {
		*w = '\0';
		return c;
	}
	for ( ; --lim > 0; w++)
		if (!isalnum(*w = getch())) {
			ungetch(*w);
			break;
		}
	*w = '\0';
	return word[0];
}

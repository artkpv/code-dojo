...
namespace pr
{
   class Solution {
        public int repeatedNumber(List<int> A) 
        {
            ...
            int bMax = (int)Math.Floor(sqrt);  // bucket max 
            int bNum = (int) ((n-1) / bMax); // buckets num
            int lbMax = bMax; // last bucket max
            if ((n-1) % bMax != 0)
            {
                bNum++;
                lbMax = (n-1) - (bMax * (bNum - 1));
            }
            //Trace.Assert(0 < lbMax && lbMax <= bNum);
            //Trace.WriteLine($"f: n={n} {sqrt} bMax={bMax} bNum={bNum} lbMax={lbMax}");
            //Trace.Indent();
            int[] b = new int[bNum];  // buckets
            int repeated = -1;
            bool found = false;
            foreach (var e in A)  // element
            {
                if (found)
                    break;
                int ebi = (int)((e-1)/bMax); // element bucket index
                //Trace.Assert(0 <= ebi && ebi < bNum, $" ebi={ebi} e={e}");
                b[ebi]++;
                if ((ebi == bNum - 1 && b[ebi] > lbMax) || (b[ebi] > bMax))
                {
                    int[] counter = new int[bMax];
                    foreach (var e2 in A)
                    {
                        if ((int)((e2-1)/bMax) != ebi)
                            continue;
                        int ci = (e2-1) - bMax*ebi;  // counter index
                        // Trace.Assert(0 <= ci && ci < bMax, $" ci={ci} e={e} e2={e2} ebi={ebi}");
                        counter[ci]++;
                        if (counter[ci] > 1)
                        {
                            repeated = e2;
            ...
            // Trace.WriteLine($"f={repeated} b: {string.Join(' ', b)} ");
            // Trace.Unindent();
            return repeated;
        }
    }
    ...
    class Program
    {
        ...
        {
            Trace.Assert(a == b, $"{a} not equals to {b}");
        }
    }
}
